...
ct %>% filter(Continent != "World") %>% ggplot(aes(Year, Total, colour=Continent)) +  geom_line()
```
...
```{r ContinentProp}
ctprop <- ct %>% mutate(PropF = PopF/total, PropM = PopM/total)
```

```{r ContinentPropPlot}
library(ggplot2)
ctprop %>%   ggplot(aes(Year, PropF), colour=Continent) %>% geom_line() +
